...
class LambdaMART:
	...
	def fit(self):
		...
		#print(  "query_indexes is:" , query_indexes)
	...
	def validate(self, data, k):
		...
		data = np.array(data)
		query_indexes = group_queries(data, 1)
		#print("test data is:", query_indexes)
		average_ndcg = []
		predicted_scores = np.zeros(len(data))
		for query in query_indexes:
			results = np.zeros(len(query_indexes[query]))
			...
			#print("query index:",query ,"DCG is:", dcg_val)
			idcg_val = ideal_dcg_k(t_results, k)
			#print("query index:",query ,"iDCG is:", idcg_val)
			ndcg_val = (dcg_val / idcg_val)
			#print("query index:",query ,"ndcg is:", ndcg_val)
			average_ndcg.append(ndcg_val)
		#print("average ndcg list:",average_ndcg )
